 ...
 */
  ...
  static final String[] dictFileList = new String[15];
      ...
      List<docQueryStruct> queryData = new ArrayList();
           ...
           for(javaFinalParser wordQ : query){
             ...
             for(int i = 0; i < wordQ.getDocumentFrequence(); i++) {
              ...
              System.out.printf("Yes word are being found %s for "
                      + "query %d "
                     + " in documents %s %n ",wordQ.getWordName(),
                     value[i][0],value[i][1]); 
                    }
                       docQueryStruct newWordD = new docQueryStruct();
                       newWordD.setDocIDInt(value[i][0]);
                       newWordD.setDocID(Integer.
                               toString(value[i][0]));
                       newWordD.addDocMapStruct(wordQ.getWordName(),
                               value[i][1]);
                       newWordD.addDocMapStructTFIDF(wordQ.getWordName(),
                           (double) value[i][1] * 
                            wordQ.getInvertDocFrequency());
                       newWordD.setTermCount(1);
                       queryData.add(newWordD);

                    } else {
                       docQueryStruct newWordUD = 
                               docSearch(queryData,qNum,dataS);
                       newWordUD.addDocMapStruct(wordQ.getWordName(),
                               value[i][1]);
                       newWordUD.addDocMapStructTFIDF(wordQ.getWordName(),
                           (double) value[i][1] * 
                             wordQ.getInvertDocFrequency());
                       newWordUD.setTermCount(newWordUD.getTermCount()
                               + 1);
